...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 class="mui-title"><spring:message code='record.usdtRecord' text='usdt记录'/></h1>
</header>
    ...
    $(function () {
        ...
        function setListData(curPageData) {
            ...
            for (var i = 0; i < curPageData.length; i++) {
                ...
                var charge = pd.CHARGE;
                if (charge == null) {
                    charge = 0
                }
                var heUser = pd.HE_NAME;
                if (!heUser) {
                    heUser = ""
                }
                var str = '';
                str += "<td>" + pd.GMT_CREATE + " </td>\n" +
                    "                    <td>" + pd.AMOUNT_TYPE + " </td>\n" +
                    "                    <td>" + pd.TAG + " " + pd.MONEY + " </td>\n" +
                    "                    <td>" + heUser + " </td>\n" +
                    "                    <td>" + charge + " </td>";
                var trDom = document.createElement("tr");
                trDom.innerHTML = str;
                listDom.appendChild(trDom);
            }
        }
        ...
        function getListDataFromNet(pageNum, pageSize, successCallback, errorCallback) {
            ...
            var url = 'front/to_usdtPage.do?num=' + pageNum + "&size=" + pageSize;
            $.ajax({
                url: url,
