...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="account/usdtList.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">ETH余额</th>
									<th class="center">USDT余额</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.USDT_ADDRESS}</td>
											<td class='center'>${var.ETH}</td>
											<td class='center'>${var.USDT}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<c:if test="${var.USDT >= 0}" >
                                                            <a class="btn btn-xs btn-success" title="转出" onclick="edit('${var.ACCOUNT_ID}', '${var.USDT}');">
                                                                USDT转出
                                                            </a>
														</c:if>
														<c:if test="${var.ETH >= 0}" >
                                                            <a class="btn btn-xs btn-success" title="转出" onclick="ethEdit('${var.ACCOUNT_ID}', '${var.ETH}');">
                                                                ETH转出
                                                            </a>
														</c:if>
													</c:if>
	...
	<script type="text/javascript">
		...
		function edit(Id, num){
			 ...
			 diag.Drag=true;
			 diag.Title ="转出";
			 diag.URL = '<%=basePath%>account/goUserEdit.do?ACCOUNT_ID='+ Id + '&num=' + num;
			 diag.Width = 450;
			 diag.Height = 355;
		...
		// eth转出
		function ethEdit(Id, num){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="转出";
			 diag.URL = '<%=basePath%>account/goEthEdit.do?ACCOUNT_ID='+ Id + '&num=' + num;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
